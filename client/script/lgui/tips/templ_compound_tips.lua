
--
-- @brief 合成弹窗
-- @author: yjg
-- @date: 2017年8月3日11:34:06
--

LogicGUI = LogicGUI or {}

LogicGUI.TemplCompoundTips = LogicGUI.TemplCompoundTips or BaseClass(GUI.Template)

LogicGUI.TemplCompoundTips.MAX = 5

function LogicGUI.TemplCompoundTips:__init(id , fun)
	self.__is_auto_load = true
	self.__fun = fun
	self.__bg_params = {type = BaseView.BGType.SOLID}	
	self.__open_callback = function()
		self:__Getwidget()
		self:__SettleData(id)
		self:Effects()
	end

	self:InitTemplate(GUI.TemplateRegistry.TEMPL_ACQUIRE_TIPS )	
end

function LogicGUI.TemplCompoundTips:__delete()
end

function LogicGUI.TemplCompoundTips:__Getwidget()
	--背景
	self.Image_bg = self:GetWidget("Image_bg")
	--特效
	self.Panel_effects = self:GetWidget("Panel_effects")
	self.Panel_effects:SetPosition(self.Panel_effects:GetPositionX() , self.Panel_effects:GetPositionY() + 10 )
	--特效文字
	self.Panel_effectText = self:GetWidget("Panel_effectText")

	--确定按钮
	self.Button_ok = Util:WidgetButton( 
										self:GetWidget("Button_ok") , 
										CPPGameLib.GetString("common_confirm") , 
										CPPGameLib.Handler(self,self.OnBtnClose) 
										,nil ,Macros.Color.white ,Macros.Color.button_yellow
										)
end

function LogicGUI.TemplCompoundTips:__SettleData( id )
	local type_ , info = ObjIDTool.GetInfo( id )

	self.iconId , iconLed = TypefaceEffects.New():Spotlight( self , self.Image_bg , info.quality or info.init_quality )
	iconLed:SetPosition( self.Image_bg:GetContentWidth()/2 - 180 , 30 )
	iconLed:SetScaleFactor( 0.85 )

	local icon = nil
	--道具
	if type_ == Macros.Game.ObjType.ITEM then

		icon = Util:Sprite( Resource.PathTool.GetItemPath(info.res_id) )
		icon:SetAnchorPoint( 0.5 , 0.5 )
		icon:SetPosition( self.Image_bg:GetContentWidth()/2 - 180 , self.Image_bg:GetContentHeight()/2 )
		self.Image_bg:AddChild( icon )
		icon:SetScaleFactor( 1.65 )
		ActionManager.GetInstance():UpDownMoveAnimation(icon , 1 , 15)
	--装备&圣器
	elseif type_ == Macros.Game.ObjType.EQUIP or type_ == Macros.Game.ObjType.TREASURE then
		icon = Util:Sprite( Resource.PathTool.GetBigIconPath(id))
		icon:SetAnchorPoint( 0.5 , 0.5 )
		icon:SetPosition( self.Image_bg:GetContentWidth()/2 - 180 , self.Image_bg:GetContentHeight()/2 )
		self.Image_bg:AddChild( icon )
		
		if type_ == Macros.Game.ObjType.EQUIP then 
			icon:SetScaleFactor( 0.65 )
		elseif type_ == Macros.Game.ObjType.TREASURE then
			icon:SetScaleFactor( 1 )
		end

		ActionManager.GetInstance():UpDownMoveAnimation(icon , 1 , 15)
	--英雄
	elseif type_ == Macros.Game.ObjType.HERO then
        --设置模型
        icon = LogicGUI.Model.New({info_id = id , parent = self.Image_bg })
        icon:SetPosition( self.Image_bg:GetContentWidth()/2 - 180 , 30 )

	end

	local lst = { 
		[Macros.Game.ObjType.ITEM] = "恭喜获得新道具",
		[Macros.Game.ObjType.EQUIP] = CPPGameLib.GetString("equip_compound_1") ,
		[Macros.Game.ObjType.TREASURE] = CPPGameLib.GetString("equip_compound_2"),
		[Macros.Game.ObjType.HERO] = CPPGameLib.GetString("equip_compound_3"),
	}

	local text = Util:Label( lst[type_] , Macros.TypefaceSize.popup , Macros.Color.golden )
	text:SetAnchorPoint( 0 , 0.5 )
	text:SetPosition( self.Image_bg:GetContentWidth()/2 + 20 , self.Image_bg:GetContentHeight()/2 + 20 )
	self.Image_bg:AddChild(text)

	local name = Util:Name( info.name , Macros.TypefaceSize.popup , info.quality or info.init_quality )
	name:SetAnchorPoint( 0.5 , 0.5 )
	name:SetPosition( text:GetPositionX() + (text:GetContentWidth()/2) , self.Image_bg:GetContentHeight()/2 - 20 )
	self.Image_bg:AddChild(name)

	--装备品级
	if type_ == Macros.Game.ObjType.EQUIP then
		name:SetAnchorPoint( 0 , 0.5 )
		name:SetPosition( text:GetPositionX() , self.Image_bg:GetContentHeight()/2 - 20 )

		local taste = Util:Label( CPPGameLib.GetString("equip_grade_" , info.taste ) , Macros.TypefaceSize.popup )
		self.Image_bg:AddChild(taste)
		PosTool.RightTo( name , taste , 10 )

	elseif type_ == Macros.Game.ObjType.HERO then
		name:SetAnchorPoint( 0 , 0.5 )
		name:SetPosition( text:GetPositionX() , self.Image_bg:GetContentHeight()/2 - 20 )

		local taste = Util:Label( CPPGameLib.GetString("hero_intelligence" , tonumber(info.Qualifications) ) , Macros.TypefaceSize.popup )
		self.Image_bg:AddChild(taste)
		PosTool.RightTo( name , taste , 10 )
	end

    local sp = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.CITY_COMMON_BTN_NEW, "examine")
	self.examine  =  Util:Button(	
									sp,
									function ( ... )
										self:__ShowTips( type_ , id , info )
									end
								)
	self.examine:SetPosition( self.Image_bg:GetContentWidth()/2 - 100 , 30 )
	self.Image_bg:AddChild(self.examine) 

end

--展示信息
function LogicGUI.TemplCompoundTips:__ShowTips( obj_type , id , info)

  --物品Tips
  if obj_type == Macros.Game.ObjType.ITEM then
    local __ , config_item = ObjIDTool.GetInfo(id)
    --装备碎片
    if config_item.type == Macros.Game.ObjType.ITEM and config_item.sub_type == Macros.Game.NormalItemSubType.EQUIPDEBRIS then
        TemplEquipInfo.GetInstance(info)
    elseif config_item.type == Macros.Game.ObjType.ITEM and config_item.sub_type == Macros.Game.NormalItemSubType.HERODEBRIS then
        CallModuleFunc(ModuleType.HERO,"OpenView",BaseViewType.HERO_HANDBOOK_DETAIL,info.item_id or id )
    else
        LogicGUI.TemplItemInfo.New(info)
    end

  --装备Tips
  elseif obj_type == Macros.Game.ObjType.EQUIP then
		local data = {}
		data.id = id
		LogicGUI.TemplEquipInfo.New(data)
  --英雄Tips
  elseif obj_type == Macros.Game.ObjType.HERO then
		CallModuleFunc(ModuleType.HERO, "OpenView", BaseViewType.HERO_HANDBOOK_DETAIL, id)
		-- LogicGUI.TemplRecruitSingleHeroView.New( id , Macros.Game.HERO_RECRUIT.MORE_RECRUIT )
  --圣器Tips
  elseif obj_type == Macros.Game.ObjType.TREASURE then
  		GUITemplateMgr:CreateTemplateInstance(GUI.TemplateRegistry.PLUNDERED_TREASURES_DETAILMESSAGE, false, id)

  elseif obj_type == Macros.Game.ObjType.AWAKEN_TOOL then
    	-- LogicGUI.TemplItemInfo.New(info)
  end

end

function LogicGUI.TemplCompoundTips:OnBtnClose()
	self:DeleteMe()
	if self.__fun then
		self.__fun()
	end
end

--特效
function LogicGUI.TemplCompoundTips:Effects()
	--特效
	local effect_id, effect = self:__CreateEffect({id = 100033}, self.Panel_effects)
	PosTool.CenterBottom(effect)	

	local effect_id, effect = self:__CreateEffect({id = 100011}, self.Panel_effectText)
	PosTool.CenterBottom(effect)
end
