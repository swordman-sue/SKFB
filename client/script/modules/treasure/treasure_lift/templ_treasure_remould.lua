...
%% @module: 圣器进化
%% @author: yjg
%% @created: 2018年7月23日10:50:51
--]]

Modules = Modules or {} 
Modules.TreasureRemouldView = Modules.TreasureRemouldView or BaseClass(BaseView)

function Modules.TreasureRemouldView:__init()
    self.__layout_name = ""  
    ...
    {
        title = CPPGameLib.GetString("treasure_evolution"),
        show_power = true,
        show_diamond = true,
        show_gold = true,
        show_level = true,
...
function Modules.TreasureRemouldView:__OpenCallback( )
    self:__Getwidget()
    self:__ShowUI()
    self:__Server()
end	

function Modules.TreasureRemouldView:__Dispose()
end

function Modules.TreasureRemouldView:__CreateWidgetFromLuaCode()
    self.widget = Util:Layout(Macros.Global.DesignSize.width, Macros.Global.DesignSize.height)
    return self.widget
end

function Modules.TreasureRemouldView:__Server()
    self:BindGlobalEvent(Macros.Event.TreasureModule.TREASURE_ADVANCE,function (_, uid)
        --刷新
        self:__SetShowUI( uid )
        local treasureInfo = CallModuleFunc(ModuleType.TREASURE, "GetTreasure", uid )
        GUITemplateMgr:CreateTemplateInstance(GUI.TemplateRegistry.TEMPL_REMOULD_SHOW , false ,treasureInfo.info_id , function( ... )
            local evolveInfo = CallModuleFunc(ModuleType.TREASURE, "ToUidGetEvolveInfo", uid )
...
function Modules.TreasureRemouldView:__Getwidget()
    --圆圈
...
function Modules.TreasureRemouldView:__ShowUI()
    local props = {}
    for i = 1 , 2 do
        props[i] = {}
        --图标
    ...
    local treasureInfo = CallModuleFunc(ModuleType.TREASURE, "GetTreasure",onTreasureInfo.uid)
    self:__SetShowUI( onTreasureInfo.uid )
end

function Modules.TreasureRemouldView:__SetShowUI( uid )
    --进化信息
    local evolveInfo = CallModuleFunc(ModuleType.TREASURE, "ToUidGetEvolveInfo", uid )
...
--现在的圣器
function Modules.TreasureRemouldView:SetNowTreasure( info )
    if not info then
        --隐藏图标
    ...
    --属性
    local evolveInfo = CallModuleFunc(ModuleType.TREASURE, "GetLiftPropPlusInfos", info )  
    local t1 = {}
...
--进化后圣器
function Modules.TreasureRemouldView:SetNexTreasure( info )
    if not info then
        --隐藏图标
    ...
    --属性
    local evolveInfo = CallModuleFunc(ModuleType.TREASURE, "GetLiftPropPlusInfos", info , true )  
    local t1 = {}
    table.insert( t1 , { type = CPPGameLib.GetString("equip_intensifyLv")  , value = info.strengthen_level } )
    for i , v in ipairs(evolveInfo) do
        local type_ , value = Util:Nature( v.type , v.value , true )
...
--消耗
function Modules.TreasureRemouldView:SetConsume( info )
    --消耗物品
    ...
    --货币
    local tab = {
                    {tp = Util:GetMoneyIconImgData( info.currency[1].type ), jg = 5 ,dx = 0.8 },
                    {zi = info.currency[1].num , dx = Macros.TypefaceSize.normal, jg = 4 },
                }
...
--消耗
function Modules.TreasureRemouldView:OnBtnBigred( )
    ...
    --货币
    local item_num = CallModuleFunc(ModuleType.ROLE,"GetPropValue" , self.consume.currency[1].type )
    if item_num < self.consume.currency[1].num then
        local text = "currency_insufficient_"..self.consume.currency[1].type
    ...
    local onTreasureInfo = CallModuleFunc(ModuleType.TREASURE, "GetOnTreasureInfo")
    local info = {}
    info.uid = onTreasureInfo.uid
    CallModuleFunc(ModuleType.TREASURE, "TreasureQualityUpReq", info ) 
end
