...
Modules.ChristmasView = Modules.ChristmasView or BaseClass(BaseView)
...
function Modules.ChristmasView:__init()
    ...
    self:__AddPreloadTemplate(GUI.TemplateRegistry.TEMPL_CHRISTMAS_SHOP_CELL)
    self:__AddPreloadTextureAtlas(Resource.UITextureAtlasType.CHRISTMAS)
    self:__AddPreloadTextureAtlas(Resource.UITextureAtlasType.CHRISTMAS,true)
end

function Modules.ChristmasView:__Dispose()
...
function Modules.ChristmasView:__RegistAllEvents()
	--错误码
	...
	self:BindGlobalEvent(Macros.Event.LimitActivityModule.CHRISTMAS_ATTACK_BOSS_RESP, function(_,attack_boss_data)
		...
		if attack_boss_data.attacker_name and attack_boss_data.attacker_name ~= role_name then				
			...
			self.__timer_event_list[timer_event_count_num] = GlobalTimerQuest:AddDelayQuest(GlobalCallbackMgr:AddCallBackFunc(
				function()
				--雪球击中特效				
				local effect_id,effect = self:__CreateEffect({id = 100093,loop = false},panel,10)
			...
			if self.__is_on_touching then
				CPPActionManager.DelayTo(self.__txt_cursnowintegralnum,0.1, GlobalCallbackMgr:AddCallBackFunc(function()
	...
	self:BindGlobalEvent(Macros.Event.LimitActivityModule.GET_SHOPDATA_BY_SHOPTYPE, function(_,shop_data,integral)
		...
		if integral then
			...
			self.__face_effects_cur:RollEx(self.__txt_cursnowintegralnum,old_value,self.__christmas_data.cur_score,0.2,nil,function (cur_value)				
				return cur_value..CPPGameLib.GetString("christmas_cur_integral_roll")
			end)
		end
		--self.__txt_cursnowintegralnum:SetText(CPPGameLib.GetString("christmas_cur_integral",self.__christmas_data.cur_score or 0))
	end)	
	...
	self:BindGlobalEvent(Macros.Event.LimitActivityModule.CHRISTMAS_SHOP_BUY_NOTIFY, function()		
		...
		self.__txt_christmashatnum:SetText(christmashat_num)
	end)	
...
function Modules.ChristmasView:__OpenCallback()
	...
	--累计积分	
	--雪球积分
	Util:WidgetLabel(self:GetWidget("Text_SnowIntegral"),CPPGameLib.GetString("christmas_snowintegral"), Macros.TypefaceSize.normal, Macros.Color.Special)
	--数字
	self.__txt_totalsnowintegralnum = self:GetWidget("Text_TotalSnowIntegralNum")
	Util:WidgetLabel(self.__txt_totalsnowintegralnum, "", Macros.TypefaceSize.normal, Macros.Color.btn)

	--当前积分
	--数字
	self.__txt_cursnowintegralnum = self:GetWidget("Text_CurSnowIntegralNum")
	Util:WidgetLabel(self.__txt_cursnowintegralnum, "", Macros.TypefaceSize.normal, Macros.Color.btn)
	...
	--遍历获取控件
	local panel_name_list = {"Panel_SnowBattle","Panel_IntegralConvert","Panel_PropConvert"}
	self.__panel_list = {}
	...
	--寻找对应的雪球
	local limit_snowball_list = {}
	for _,v in ipairs(CPPGameLib.GetConfig("ChristmasBoss",nil,nil,true)) do
		for i,snowball_id in ipairs(v.limit_item_list) do
			limit_snowball_list[snowball_id] = snowball_id
		end
	end
	self.__snowball_id_list = {}
	for snowball_id,_ in pairs(CPPGameLib.GetConfig("ChristmasSnowBall",nil,nil,true)) do
		if limit_snowball_list[snowball_id] then
			table.insert(self.__snowball_id_list,snowball_id)
		end
	end
	table.sort(self.__snowball_id_list,function (snowball_id1,snowball_id2)
		return snowball_id1 < snowball_id2
	end)
	--滚动容器数据
	...
	--设置Tab
	local sp_1 = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.CHRISTMAS, "btn_unselected")
    local sp_2 = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.CHRISTMAS, "btn_selected")
	self.__tab_params = {
	    {sp_2,sp_1},
	    {  
	    	"",
    ...
    --按钮特殊处理
    local tab_sp_name = {"snow_man","christmas_tree","christmas_hat"}
    for i=1,3 do
    	local btn = self.__tab:GetBtn(i)
    	if btn then
    		--图片
    		local sp = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.CHRISTMAS, tab_sp_name[i])
    		local sprite = Util:Sprite(sp)
    		sprite:SetAnchorPoint(0,0.5)
    		btn:AddChild(sprite)
    		PosTool.LeftCenter(sprite,2,0)
    		--文字
    		local label = Util:Label(CPPGameLib.GetString("christmas_tab_txt"..i), Macros.TypefaceSize.headlineII, Macros.Color.lilyWhite)
    		btn:AddChild(label)
    		PosTool.Center(label,25,0)
    	end
    end
    --圣诞帽
    local image_christmashaticon = self:GetWidget("Image_ChristmasHatIcon")
    --图片
    local sp = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.CHRISTMAS,"christmas_hat")
    image_christmashaticon:SetImage(sp)
    --圣诞帽数量
    self.__txt_christmashatnum = self:GetWidget("Text_ChristmasHatNum")
    local christmashat_num = ObjIDTool.GetOwnNum(ConfigAdapter.Common.GetChristmasHatId())
	Util:WidgetLabel(self.__txt_christmashatnum,christmashat_num, Macros.TypefaceSize.largish, Macros.Color.btn)
    --文字容器table
...
function Modules.ChristmasView:ChangeTag(index)
	--设置控件显隐
	...
	--根据类型选择界面
	if index == TAB_TYPE.SNOW_BATTLE then
		--雪球大战
...
--设置攻击Boss后的数据显示
function Modules.ChristmasView:HandleAfterAttackBossData(dead_boss_type)
	if self.__face_effects then
		self.__face_effects:StopAllRolling()
		--self.__face_effects = nil
	end
	--累计积分
	self.__txt_totalsnowintegralnum:SetText(CPPGameLib.GetString("christmas_total_integral",self.__christmas_data.total_score or 0))
	--当前积分
	self.__txt_cursnowintegralnum:SetText(CPPGameLib.GetString("christmas_cur_integral",self.__christmas_data.cur_score or 0))
	--BOSS相关信息
	for boss_type=1,2 do
		...
		if dead_boss_type and not is_alive and dead_boss_type == boss_type then
			...
			--BOSS击杀
			local effect_id,effect = self:__CreateEffect({id = 100094},panel_bossmodule,500)
...
--冰雪大战
function Modules.ChristmasView:HandleSnowBattle()
	if not self.__snow_battle_init then
		--相同的func
		self.__same_act_func = function ()		
			--设置各个雪球是否能点击
			self:SetSnowBallTouchEnabled()	
			CPPActionManager.StopAllActions(self.__txt_cursnowintegralnum)
		...
		self.__boss_module1:SetScaleFactor(low_config.scale_factor)
		...
		self.__boss_module2:SetScaleFactor(high_config.scale_factor)
		...
		for i,name in ipairs(self.__snow_ball_type_list) do
			...
			function(x,y,in_cd)			
				if not in_cd then
					...
					self.__is_on_touching = false 
					self:CheckIsSatisfyConditional(i,i <= 2 and BOSS_TYPE.LOW_BOSS or BOSS_TYPE.HIGH_BOSS,self.__snowball_id_list[i])
					self.__delay_timer = GlobalTimerQuest:AddDelayQuest(GlobalCallbackMgr:AddCallBackFunc(function()
				    	self.__delay_timer = nil
				    	if self:CheckIsSatisfyConditional(i,i <= 2 and BOSS_TYPE.LOW_BOSS or BOSS_TYPE.HIGH_BOSS,self.__snowball_id_list[i]) then
...
--检测是否有足够的雪球消耗
function Modules.ChristmasView:CheckIsSatisfyConditional(index,boss_type,snow_ball_id)
...
--设置宝箱显隐
function Modules.ChristmasView:SetBoxImageVisible()
	--低级宝箱
...
--设置各个雪球的数量
function Modules.ChristmasView:SetSnowBallNum()
	--设置扔雪球
	for i,name in ipairs(self.__snow_ball_type_list) do
		--创建图标
		...
		local snow_ball_num = ObjIDTool.GetOwnNum(self.__snowball_id_list[i])
		self["__icon_"..name]:SetData({id = self.__snowball_id_list[i],num = snow_ball_num})
		self["__icon_"..name]:ShowItemName(true)
	end
end

--设置各个雪球是否能点击
function Modules.ChristmasView:SetSnowBallTouchEnabled(index)
	for i,name in ipairs(self.__snow_ball_type_list) do
...
--攻击Boss后的回调
function Modules.ChristmasView:HandleAttackBossResp(attack_boss_data,pos_data)
	...
	if pos_data then
		...
		self.__face_effects_cur:RollEx(self.__txt_cursnowintegralnum,self.__christmas_data.cur_score - attack_boss_data.damage,self.__christmas_data.cur_score,0.2,nil,function (cur_value)
			return cur_value..CPPGameLib.GetString("christmas_cur_integral_roll")
		end)
		--总积分
		...
		self.__face_effects_total:RollEx(self.__txt_totalsnowintegralnum,self.__christmas_data.total_score - attack_boss_data.damage,self.__christmas_data.total_score,0.2,nil,function (cur_value)
...
--积分兑换
function Modules.ChristmasView:HandleChristmasShop(shop_data)
	--积分滚动容器
	if not self[self.__scrollview_data_list[self.__seleceted_index].name] then
		...
		local params = 
	    {
	        item_class = Modules.TemplChristmasShopItem,
	        item_width = 353,
	        item_height = 110,
	        row = 4,
	        col = 2,
...
--复活倒计时
function Modules.ChristmasView:BossResurgenceCountDown(boss_type)
...
--复活倒计时
function Modules.ChristmasView:UpdateCountDownTimeTxt(boss_type)
	--BOSS配置
	local boss_config = CPPGameLib.GetConfig("ChristmasBoss",boss_type)
	local severTime = CallModuleFunc(ModuleType.TIME_SYNC, "GetServerTime")
	local boss_death_time = boss_type == BOSS_TYPE.LOW_BOSS and self.__christmas_data.lower_boss_death_time or self.__christmas_data.high_boss_death_time
