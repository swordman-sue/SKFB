...
Modules.SpringFestivalView = Modules.SpringFestivalView or BaseClass(BaseView)
...
function Modules.SpringFestivalView:__init()
	self.__layout_name = "spring_festival"  
    self.__bg_params = {type = BaseView.BGType.IMAGE, image_path = Resource.PathTool.GetUIResPath("bigbg/bg_sp_springfestival")}
    self.__title_bar_params = 
    {
        title = CPPGameLib.GetString("springfestival_title"),
        close_func = function()
        	--请求离开场景       	
    ...
    self:__AddPreloadTextureAtlas(Resource.UITextureAtlasType.SPRING_FESTIVAL)
    self:__AddPreloadTextureAtlas(Resource.UITextureAtlasType.SPRING_FESTIVAL,true)
end

function Modules.SpringFestivalView:__Dispose()
...
function Modules.SpringFestivalView:__RegistAllEvents()
	--错误码
	...
	self:BindGlobalEvent(Macros.Event.LimitActivityModule.CHRISTMAS_ATTACK_BOSS_RESP, function(_,attack_boss_data)
		...
		if attack_boss_data.attacker_name and attack_boss_data.attacker_name ~= role_name then				
			...
			local sp_effect
			print("ldx","UI_firecrackerUI_firecrackerUI_firecracker UI_firecrackerUI_firecracker11111111111111111111 ")
	        snowball_data.effect_id, sp_effect = self:__CreateEffect({res_name = "UI_firecracker",type = Effect.EffectType.SKELETON, act_index = RenderObj.ActionIndex.IDLE},panel,30000)
	        sp_effect:SetAnchorPoint(0.5,0.5)
			sp_effect:SetPosition(snowball_data.random_beg_posx,snowball_data.random_beg_posy)
			...
			self.__timer_event_list[timer_event_count_num] = GlobalTimerQuest:AddDelayQuest(GlobalCallbackMgr:AddCallBackFunc(
				function()
				--雪球击中特效				
				local effect_id,effect = self:__CreateEffect({id = 100097,loop = false},panel,10)
...
function Modules.SpringFestivalView:__OpenCallback()
	...
	self:SetUpdateFunc(function (elapse_time)
		...
		for i,snowball_data in ipairs(self.__snow_ball_list) do
	        ...
	        local effect 
	        if snowball_data.effect_id then
	        	effect = GlobalEffectMgr:GetEffect(snowball_data.effect_id)
	        	if effect then
			        effect:SetScaleFactor(2*(snowball_data.random_fly_time - snowball_data.smooth_elapse_time) + 0.5)
			        effect:SetPosition(x,y)
			    end
		    end
	        if snowball_data.smooth_elapse_time >= snowball_data.random_fly_time then
	        	if snowball_data.effect_id then
	        		self:__DelEffect(snowball_data.effect_id)
					snowball_data.effect_id = nil
	        	end
	...
	--累计积分	
	--雪球积分
	Util:WidgetLabel(self:GetWidget("Text_FirecrackerIntegral"),CPPGameLib.GetString("springfestival_firecrackerintegral"), Macros.TypefaceSize.normal, Macros.Color.Special)
	--数字
	...
	for _,v in ipairs(CPPGameLib.GetConfig("ChristmasBoss",nil,nil,true)) do
		for i,firecracker_id in ipairs(v.limit_item_list) do
	...
	--设置Tab
	local sp_1 = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.SPRING_FESTIVAL, "btn_selected")
    local sp_2 = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.SPRING_FESTIVAL, "btn_unselected")
	self.__tab_params = {
	    {sp_2,sp_1},
	    {  
	    	"",
    ...
    --按钮特殊处理
    local tab_sp_name = {"firecracker","best_gift","lantern"}
    local tab_sp_txt = {"drive_monster","integral_convert","prop_convert"}
    for i=1,3 do
    	local btn = self.__tab:GetBtn(i)
    	if btn then
    		--图片
    		local sp = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.SPRING_FESTIVAL, tab_sp_name[i])
    		local sprite = Util:Sprite(sp)
    		sprite:SetAnchorPoint(0,0.5)
    		btn:AddChild(sprite)
    		PosTool.LeftCenter(sprite,-8,-3)
    		--文字
    		sp = Resource.LoadTool.GetImageData(Resource.UITextureAtlasType.SPRING_FESTIVAL, tab_sp_txt[i],true)
...
function Modules.SpringFestivalView:ChangeTag(index)
	--设置控件显隐
...
--设置攻击Boss后的数据显示
function Modules.SpringFestivalView:HandleAfterAttackBossData(dead_boss_type)
	if self.__face_effects then
...
--冰雪大战
function Modules.SpringFestivalView:HandleSnowBattle()
				...
				if not in_cd then
					...
					if not self:CallModuleFunc("GetActivityOpen",Macros.Game.SystemID.CHRISTMAS_ACTIVITY) then
						...
						if not self.__is_check_open then
							CallModuleFunc(ModuleType.MAIN_CITY,"ResetRegionEntrysBySystemID",Macros.Game.SystemID.SPRING_FESTIVAL)
...
--检测是否有足够的雪球消耗
function Modules.SpringFestivalView:CheckIsSatisfyConditional(index,boss_type,snow_ball_id)
...
--设置宝箱显隐
function Modules.SpringFestivalView:SetBoxImageVisible()
	--低级宝箱
...
--设置各个雪球的数量
function Modules.SpringFestivalView:SetSnowBallNum()
	--设置扔雪球
	for i,name in ipairs(self.__snow_ball_type_list) do
		--创建图标
...
--设置各个雪球是否能点击
function Modules.SpringFestivalView:SetSnowBallTouchEnabled(index)
	for i,name in ipairs(self.__snow_ball_type_list) do
...
--攻击Boss后的回调
function Modules.SpringFestivalView:HandleAttackBossResp(attack_boss_data,pos_data)
...
--积分兑换
function Modules.SpringFestivalView:HandleChristmasShop(shop_data)
	--积分滚动容器
...
--复活倒计时
function Modules.SpringFestivalView:BossResurgenceCountDown(boss_type)
...
--复活倒计时
function Modules.SpringFestivalView:UpdateCountDownTimeTxt(boss_type)
	--BOSS配置
	local boss_config = CPPGameLib.GetConfig("ChristmasBoss",boss_type)
	local severTime = CallModuleFunc(ModuleType.TIME_SYNC, "GetServerTime")
	local boss_death_time = boss_type == BOSS_TYPE.LOW_BOSS and self.__christmas_data.lower_boss_death_time or self.__christmas_data.high_boss_death_time
