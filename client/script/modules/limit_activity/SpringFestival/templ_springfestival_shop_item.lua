		...
		if self.__data.shop_type == Macros.Game.SHOP.CHRISTMAS_INTEGRAL then
			...
			if not self.__data.is_satisfy_level then
				...
				if self.__data.total_score >= self.__data.need_history_score then
					Util:WidgetLabel(txt_conditional,CPPGameLib.GetString("springfestival_firecrackerintegral_shop",self.__data.cost_score), Macros.TypefaceSize.slightly,Macros.Color.btn)	
				...
				--所需积分
				Util:WidgetLabel(txt_conditional,self.__data.total_score >= self.__data.need_history_score and CPPGameLib.GetString("springfestival_firecrackerintegral_shop",self.__data.cost_score) or CPPGameLib.GetString("springfestival_shop_needintegral",self.__data.need_history_score),
...
function Modules.TemplSpringFestivalShopItem:BuySomeThing()
	...
	if self.__data.shop_type == Macros.Game.SHOP.CHRISTMAS_PROP then
		...
		if integral < self.__data.cost_score then
			GlobalTipMsg.GetInstance():Show(CPPGameLib.GetString("springfestival_shop_noenough_firecracker_integral"))			
			return
		end
		could_buy_num = integral/self.__data.cost_score
		use_num = self.__data.cost_score
		cur_data.sp_txt = "springfestival_shop_total_need"
	end
