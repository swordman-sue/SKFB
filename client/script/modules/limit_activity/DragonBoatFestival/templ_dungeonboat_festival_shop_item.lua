...
Modules.TemplSpringFestivalShopItem = Modules.TemplSpringFestivalShopItem or BaseClass(GUI.ScrollItem)


function Modules.TemplSpringFestivalShopItem:__init()
	self:InitTemplate(GUI.TemplateRegistry.TEMPL_SPRINGFESTIVAL_SHOP_CELL)
end

function Modules.TemplSpringFestivalShopItem:__delete()
	
end

function Modules.TemplSpringFestivalShopItem:SetData(data)
	...
	if self.__data.shop_type == Macros.Game.SHOP.CHRISTMAS_PROP then
		...
		image_money:IgnoreContentAdaptWithSize(true)
		image_money:SetImage(sp,TextureResType.LOCAL)
		image_money:SetPosition(image_money:GetPositionX(),image_money:GetPositionY()+5)
		image_money:SetScaleFactor(0.5)
		--价格
		...
		if self.__data.shop_type == Macros.Game.SHOP.CHRISTMAS_INTEGRAL then
			...
			if not self.__data.is_satisfy_level then
				...
				if self.__data.total_score >= self.__data.need_history_score then
					Util:WidgetLabel(txt_conditional,CPPGameLib.GetString("dragonboat_festival_firecrackerintegral_shop",self.__data.cost_score), Macros.TypefaceSize.slightly,Macros.Color.btn)	
				...
				--所需积分
				Util:WidgetLabel(txt_conditional,self.__data.total_score >= self.__data.need_history_score and CPPGameLib.GetString("dragonboat_festival_firecrackerintegral_shop",self.__data.cost_score) or CPPGameLib.GetString("dragonboat_festival_shop_needintegral",self.__data.need_history_score),
...
function Modules.TemplSpringFestivalShopItem:BuySomeThing()
	...
	if self.__data.shop_type == Macros.Game.SHOP.CHRISTMAS_PROP then
		...
		if integral < self.__data.cost_score then
			GlobalTipMsg.GetInstance():Show(CPPGameLib.GetString("dragonboat_festival_shop_noenough_firecracker_integral"))			
			return
		end
		could_buy_num = integral/self.__data.cost_score
		use_num = self.__data.cost_score
		cur_data.sp_txt = "dragonboat_festival_shop_total_need"
	end
