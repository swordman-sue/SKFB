...
--英雄副本关卡模板
Modules.TemplHeroDungeonGameLevel = Modules.TemplHeroDungeonGameLevel or BaseClass(GUI.Template)

function Modules.TemplHeroDungeonGameLevel:__init(parent_node)
	self:InitTemplate(GUI.TemplateRegistry.HERO_DUNGEON_GAMELEVEL, parent_node)
...
function Modules.TemplHeroDungeonGameLevel:__delete()
...
function Modules.TemplHeroDungeonGameLevel:SetData(data)
	...
	if not gamelevel_info then
		...
		self.__is_on_gray = true
	else
		if self.__is_on_gray then
			self.__is_on_gray = false
	...
	--已挑战标记	
	self.__img_challenged:SetVisible(gamelevel_info and gamelevel_info.battle_times > 0)
...
function Modules.TemplHeroDungeonGameLevel:__ShowEffect()
	local gamelevel_info = self.__data.gamelevel_info
	local flag = Cookies.Get(Cookies.LevelType.ROLE, string.format("%s_%d", Macros.Setting.HeroDungeonGameLevelEffect, self.__data.gamelevel_id)) ~= "1"
	if gamelevel_info and gamelevel_info.battle_times == 0 then
		local effect
		if flag then
	        self.__light_effect_id, effect = self:__CreateEffect({id = 100003,loop = false}, self.__root_widget,3)
			PosTool.Center(effect,0,-75)				
			Cookies.Set(Cookies.LevelType.ROLE, string.format("%s_%d", Macros.Setting.HeroDungeonGameLevelEffect, self.__data.gamelevel_id), "1")
		end	
...
function Modules.TemplHeroDungeonGameLevel:__ShowBox()
	local config_gamelevel = CPPGameLib.GetConfig("GameLevel", self.__data.gamelevel_id)
...
function Modules.TemplHeroDungeonGameLevel:DeleteSpecialEffect()
...
--新手指引特殊操作检测
function Modules.TemplHeroDungeonGameLevel:CheckSectionOperateSP()
	--选择英雄副本关卡（关卡ID）
	CallModuleFunc(ModuleType.LEAD, "CheckSectionOperateSP", Macros.Game.LeadOperateSpecialID.HERO_DUNGEON_SELECT_GAMELEVEL, self:GetNode(), self.__data.gamelevel_id)				
end

function Modules.TemplHeroDungeonGameLevel:__HandleGamelevelClick()
	local gamelevel_info = self.__data.gamelevel_info
	if not gamelevel_info then
	...
	--打开挑战界面		
	Modules.TemplHeroDungeonGameLevelChallenge.New(self.__data)		
end

function Modules.TemplHeroDungeonGameLevel:__HandleBoxClick()
	--打开宝箱信息界面
	local config_gamelevel = CPPGameLib.GetConfig("GameLevel", self.__data.gamelevel_id)
	local params = {}
	...
	params.fb_type = Macros.Game.FBType.HERO_DUNGEON
