...
LogicGUI.TemplHeroResolve = LogicGUI.TemplHeroResolve or BaseClass( LogicGUI.TemplRecycleResolve )

function LogicGUI.TemplHeroResolve:__init()
    self.key = true
    self.modes = {}
    self.lab_name = {}
    self.Button_remove = {}
...
function LogicGUI.TemplHeroResolve:__CloseCallback()
    --商店字样
    ...
    print("xy" , "英雄分解界面--------------")
    if CallModuleFunc(ModuleType.RECYCLE , "GetOnTab" ) == 1 then
        CallModuleFunc(ModuleType.RECYCLE, "RemoveAllOnData")
    end
end

function LogicGUI.TemplHeroResolve:__SetShowUI()

    --商店字样
    self.shopText = Util:Name( CPPGameLib.GetString("heroShop_shop"))
    ...
    self.labCenter =  Util:LabCenter( { 
                                        { zi = CPPGameLib.GetString("role_info_hero_soul").. CPPGameLib.GetString("common_colon") , mb = true , dx = Macros.TypefaceSize.minimum } ,
                                        { tp = Util:GetMoneyIconImgData(Macros.Game.RolePropType.HERO_SOUL) , dx = 0.7 } ,
                                        { zi = userDate.hero_soul , mb = true , dx = Macros.TypefaceSize.minimum , jg = 5 } , 
                                    } )
    ...
    --提示
    local tips = GlobalCustomizedLogicAdapterMgr:Call("GetHeroRecycleTips")
    self.lab = Util:RichText(tips or CPPGameLib.GetString("recycle_heroTips") ,Macros.TypefaceSize.slightly)
...
function LogicGUI.TemplHeroResolve:__UIShowUI()
end

--按钮回调
function LogicGUI.TemplHeroResolve:OnAddBtnLst()
    CallModuleFunc(ModuleType.RECYCLE, "RemoveAllOnData")
    CallModuleFunc(ModuleType.RECYCLE, "OpenView", BaseViewType.RESOLVE_LST , Macros.Game.ObjType.HERO)
end

--商店按钮
function LogicGUI.TemplHeroResolve:OnButtonShop( ... )
    CallModuleFunc(ModuleType.HERO_SHOP, "OpenView", BaseViewType.HERO_SHOP)
end

--设置选中的按钮
function LogicGUI.TemplHeroResolve:ShowOnData( info )
    self.key = true
    ...
    for i , v in ipairs( lst ) do
        ...
        --模型
        self.modes[i] = LogicGUI.Model.New({info_id = info_.model_id, parent = self["Panel_itemPos"..i] })
        self.modes[i]:SetPosition( self["Panel_itemPos"..i]:GetContentWidth()/2 , -50 )
        self.modes[i]:SetScaleFactor(0.85)
        ...
        self.lab_name[i]:SetPosition( self["Panel_itemPos"..i]:GetContentWidth()/2 , -50 )
        self["Panel_itemPos"..i]:AddChild(self.lab_name[i])

        --设置红点
        self.Button_remove[i] = Util:Button(sp)
        self.Button_remove[i]:SetPosition( self["Panel_itemPos"..i]:GetContentWidth() + 40 , self["Panel_itemPos"..i]:GetContentHeight() + 120)
...
function LogicGUI.TemplHeroResolve:RemoveUi( )
    --重置选择控件 
    for i=1,10 do
...
-- 自动添加
function LogicGUI.TemplHeroResolve:OnAuto()
    CallModuleFunc(ModuleType.RECYCLE, "RemoveAllOnData")
    local list = CallModuleFunc(ModuleType.RECYCLE, "DateLst", Macros.Game.ObjType.HERO )
    local index = 0
    ...
    if index == 0 then 
        GlobalTipMsg.GetInstance():Show( CPPGameLib.GetString("recycle_insufficient_1") )
    end
end

--分解特效
function LogicGUI.TemplHeroResolve:ResolveEffects( info_ )
