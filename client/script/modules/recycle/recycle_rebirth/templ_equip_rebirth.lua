...
-- @brief 装备重生界面
-- @author: yjg
-- @date: 2017年11月15日18:31:08

LogicGUI = LogicGUI or {}

LogicGUI.TemplEquipRebirthe = LogicGUI.TemplEquipRebirthe or BaseClass( LogicGUI.TemplRecycleRebirth )

function LogicGUI.TemplEquipRebirthe:__init()
    self.key = true
end

function LogicGUI.TemplEquipRebirthe:__CloseCallback()
...
function LogicGUI.TemplEquipRebirthe:__SetShowUI()
    --提示
    self.lab = Util:RichText( CPPGameLib.GetString("recycle_rebirthEquipTips") ,Macros.TypefaceSize.slightly)
...
function LogicGUI.TemplEquipRebirthe:__UIShowUI()
end

--按钮回调
function LogicGUI.TemplEquipRebirthe:OnAddBtnLst()
    CallModuleFunc(ModuleType.RECYCLE, "RemoveAllOnData")
    CallModuleFunc(ModuleType.RECYCLE, "OpenView", BaseViewType.RECYCLE_LST , Macros.Game.ObjType.EQUIP)
end

--设置选中的按钮
function LogicGUI.TemplEquipRebirthe:ShowOnData( info )
...
function LogicGUI.TemplEquipRebirthe:RemoveUi( )
    LogicGUI.TemplRecycleRebirth.RemoveUi(self )
    
    --重置选择控件 
    if self.modes then
        self.modes:RemoveSelf(true)
        self.modes = nil
    end
...
--分解特效
function LogicGUI.TemplEquipRebirthe:ResolveEffects( info_ )
    ...
    if moneyNum >= self.reset then 
        if self.modes then
            ...
            local effect_id , effect_con = self:__CreateEffect({res_name = "UI_huishou_huanrao" , loop = false }, self.Panel_rebirthItemPos)
            effect_con:SetPosition( self.Panel_rebirthItemPos:GetContentWidth()/2 , self.Panel_rebirthItemPos:GetContentHeight()/2)
