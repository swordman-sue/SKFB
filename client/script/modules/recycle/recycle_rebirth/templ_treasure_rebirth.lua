...
-- @brief 圣器重生界面
-- @author: yjg
-- @date: 2017年11月15日18:42:07

LogicGUI = LogicGUI or {}

LogicGUI.TemplTreasureRebirthe = LogicGUI.TemplTreasureRebirthe or BaseClass( LogicGUI.TemplRecycleRebirth )

function LogicGUI.TemplTreasureRebirthe:__init()
    self.key = true
    -- self.modes = {}
    -- self.lab_name = {}
    -- self.Button_remove = {}
...
function LogicGUI.TemplTreasureRebirthe:__CloseCallback()
...
function LogicGUI.TemplTreasureRebirthe:__SetShowUI()
    --提示
    self.lab = Util:RichText( CPPGameLib.GetString("recycle_rebirthTreasureTips") ,Macros.TypefaceSize.slightly)
...
function LogicGUI.TemplTreasureRebirthe:__UIShowUI()
end

--按钮回调
function LogicGUI.TemplTreasureRebirthe:OnAddBtnLst()
    CallModuleFunc(ModuleType.RECYCLE, "RemoveAllOnData")
    CallModuleFunc(ModuleType.RECYCLE, "OpenView", BaseViewType.RECYCLE_LST , Macros.Game.ObjType.TREASURE)
end

--设置选中的按钮
function LogicGUI.TemplTreasureRebirthe:ShowOnData( info )
...
function LogicGUI.TemplTreasureRebirthe:RemoveUi( )
...
--分解特效
function LogicGUI.TemplTreasureRebirthe:ResolveEffects( info_ )
