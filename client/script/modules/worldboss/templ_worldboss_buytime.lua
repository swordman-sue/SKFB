...
LogicGUI.TemplWorldbossBuytime = LogicGUI.TemplWorldbossBuytime or BaseClass(GUI.Template)

function LogicGUI.TemplWorldbossBuytime:__init()
...
function LogicGUI.TemplWorldbossBuytime:__delete()
end

function LogicGUI.TemplWorldbossBuytime:__Getwidget()
	--大背景
...
function LogicGUI.TemplWorldbossBuytime:__ShowUI()
	--标题
	...
	local nowTime = Util:LabCenter( {
									...
									{ zi = CallModuleFunc(ModuleType.WORLDBOSS, "GetNowChallengeTime") , dx = Macros.TypefaceSize.largish , ys = Macros.Color.content },
								} )
	...
	local mayBuyTime = Util:LabCenter( {
									...
									{ zi = CallModuleFunc(ModuleType.WORLDBOSS, "GetWorldBossBuyTime") , dx = Macros.TypefaceSize.largish , ys = Macros.Color.content },
								} )
	...
	self.cnf = ConfigAdapter.Common.GetWorldBossBuyPrice()
	self:OnButtonAdd(1)
end

--关闭按钮
function LogicGUI.TemplWorldbossBuytime:OnButtonClose()
	self:DeleteMe()
end

--购买按钮
function LogicGUI.TemplWorldbossBuytime:OnButtonBuy()
	local userDate = CallModuleFunc(ModuleType.ROLE , "GetRoleInfo")
	local max = CallModuleFunc(ModuleType.WORLDBOSS, "GetWorldBossBuyTime")

	print("xy" , "=================" , userDate.diamond , self.num , max , 0 )
	if userDate.diamond >= (self.num or 0) and max ~= 0 then
		local info = {}
		info.buy_times = self.pitchNum
		CallModuleFunc(ModuleType.WORLDBOSS, "WorldBossBattleTimesBuyReq",info)
		self:DeleteMe()	
	elseif max == 0 then		
        local type_ , value_ = CallModuleFunc(ModuleType.VIP , "ToTypeGetResetItem" , Macros.Game.VIPType.WORLDBOSS_BUY_TIME )
        if type_ == true then
			...
			GlobalTipMsg.GetInstance():Show( CPPGameLib.GetString("worldBoss_nobuy") )
		end
	else
...
--添加按钮
function LogicGUI.TemplWorldbossBuytime:OnButtonAdd(index)
	local max = CallModuleFunc(ModuleType.WORLDBOSS, "GetWorldBossBuyTime")
	if self.pitchNum >= max or max <= 0  then
		return
...
--减少按钮
function LogicGUI.TemplWorldbossBuytime:OnButtonSub(index)
	print("xy" , "减少按钮",index)
...
--价格
function LogicGUI.TemplWorldbossBuytime:SetPrice()
	local used = CallModuleFunc(ModuleType.WORLDBOSS, "GetWorldBossUsedUpBuyTime")
