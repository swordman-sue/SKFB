...
std::string MD5String(const char* srcString)
{
	...
	unsigned int len = strlen (srcString);
	int i;

	MD5Init( &context );
	MD5Update( &context, (unsigned char*)srcString, len );
...
{
	std::string md5String = MD5String( string );
	return strcmp( md5string, md5String.c_str() ) == 0;
}