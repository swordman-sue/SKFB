...
void MsgSdkData::ToCobject(BaseMsgData^ msg)
{
	int code = msg->getCode();
	this->code = code;

	std::string data = ZQ::StrUtil::WindowsPlatformToUTF8(msg->getData());
	this->data = data;

	int msgType = (int)msg->getMsgType();
	this->msg_type = MsgSdkData::Msg_Type(msgType);
}

MsgSdkData* MsgSdkData::CreateMessage(MSG_CODE msg_code)
{
	return new MsgSdkData();
}

MsgAsyncLoadImageData* MsgAsyncLoadImageData::CreateMessage(MSG_CODE msg_code, ZQ::UINT32 luaFuncId)
{
	return new MsgAsyncLoadImageData(luaFuncId);
}