...
#include "helper/stdlibex.h"
#include "helper/stringex.h"
#include "helper/mappingex.h"
...
std::string Ldid::ldid(const std::string file_path)
{
    union {
        uint16_t word;
        uint8_t byte[2];
...
//    }
	...
	for (bool _stop(true); _stop; )
		for (const __typeof__(files) &_list = (files); _stop; _stop = false)
			for (std::vector<std::string>::const_iterator file = _list.begin(); file != _list.end(); ++file)
                //_foreach (file, files)
			try {
        ...
        std::vector<FatMachHeader> tmp_header = fat_header.GetMachHeaders();
        for (bool _stop(true); _stop; )
            for (const __typeof__(tmp_header) &_list = (tmp_header); _stop; _stop = false)
                for (std::vector<FatMachHeader>::const_iterator mach_header = _list.begin(); mach_header != _list.end(); ++mach_header)
                {
       // _foreach (mach_header, fat_header.GetMachHeaders()) {
            //if (flag_A) {
            //    if (mach_header.GetCPUType() != flag_CPUType)
            ...
            std::vector<struct load_command *> tmp_load_command = mach_header->GetLoadCommands();
            for (bool _stop(true); _stop; )
                for (const __typeof__(tmp_load_command) &_list = (tmp_load_command); _stop; _stop = false)
                    for (std::vector<struct load_command *>::const_iterator load_command = _list.begin(); load_command != _list.end(); ++load_command)
                    {
           // _foreach (load_command, mach_header->GetLoadCommands()) {
                uint32_t cmd(mach_header->Swap((*load_command)->cmd));
            ...
            if (flag_e) {
                ...
                for (size_t index(0); index != Swap(super->count); ++index)
                    if (Swap(super->index[index].type) == CSSLOT_ENTITLEMENTS) {
                        ...
                        std::string tmp=std::string((char*)(entitlements + 1), Swap(entitlements->length) - sizeof(struct Blob));
                        file_content += tmp;
                    }
            }
