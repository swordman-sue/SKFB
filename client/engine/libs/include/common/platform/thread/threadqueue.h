#ifndef COMMON_THREADQUEUE_H
#define COMMON_THREADQUEUE_H

#include <stdlib.h>
#include <memory>
#include <assert.h>
#include "mutex.h"
#include "semaphorex.h"

/*
	标准的生产消费队列，使用信号量进行PV操作，分开读锁写锁
...
public:
	...
	{		
		//buffer要比队列长度大1，否则read_ptr与write_ptr在队列满时会重叠
		unsigned long size = buffersize + 1;			
