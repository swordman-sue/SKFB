...
#include "luaengine/luaprofile.h"

char g_exe_file_path[MAX_PATH];
char g_dump_file_path[MAX_PATH];
		...
		if (pDump)
		{
			...
			if (hFile)
			{
				...
				::_snprintf(temp, sizeof(temp), 
					...
					"Exception Code: %X, at address %p", 
					g_dump_file_path, g_dump_file_name, pRec->ExceptionCode, pRec->ExceptionAddress);
			} 
			else
			{
...
void start_except_catch()
{
	...
	WideCharToMultiByte(CP_ACP, 0, wszPath, GetCurrentDirectoryW(sizeof(wszPath), wszPath), g_dump_file_path, MAX_PATH, NULL, NULL);
	strcat(g_dump_file_path, "\\");
