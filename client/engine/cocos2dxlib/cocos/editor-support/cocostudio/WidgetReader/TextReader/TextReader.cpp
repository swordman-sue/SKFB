...
namespace cocostudio
{
    ...
    void TextReader::setPropsFromJsonDictionary(Widget *widget, const rapidjson::Value &options)
    {
		...
		{
			fontName = WidgetReader::DefaultFontName;
		}
    ...
    void TextReader::setPropsWithFlatBuffers(cocos2d::Node *node, const flatbuffers::Table *textOptions)
    {
		...
		std::string fontPath = resourceData->path()->c_str();

		//by swordman sue 统一处理字体 2016-11-18
		if (WidgetReader::DefaultFontName != "")
		{
			fontPath = WidgetReader::DefaultFontName;
		}

		if (!fontPath.empty() && FileUtils::getInstance()->isFileExist(fontPath))
        {
			label->setFontName(fontPath);
        }
        else
        {
			label->setFontName(fontPath);
