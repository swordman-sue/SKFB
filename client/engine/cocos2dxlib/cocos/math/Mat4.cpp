...
// add by zhuyi 2014-08-27
Mat4* Mat4::Mat4MakeMat4(Mat4* pOut, const  Quaternion* pQ, const  Vec3* translation,
							 const  Vec3* scale)
{
	pOut->m[0] = scale->x * (1.0f - 2.0f * (pQ->y * pQ->y + pQ->z * pQ->z ));
	pOut->m[1] = scale->y * (2.0f * (pQ->x * pQ->y + pQ->z * pQ->w));
	pOut->m[2] = scale->z * (2.0f * (pQ->x * pQ->z - pQ->y * pQ->w));
	pOut->m[3] = 0.0f;

	// Second row
	pOut->m[4] = scale->x * (2.0f * ( pQ->x * pQ->y - pQ->z * pQ->w ));
	pOut->m[5] = scale->y * (1.0f - 2.0f * ( pQ->x * pQ->x + pQ->z * pQ->z ));
	pOut->m[6] = scale->z * (2.0f * (pQ->z * pQ->y + pQ->x * pQ->w ));
	pOut->m[7] = 0.0f;

	// Third row
	pOut->m[8] = scale->x * (2.0f * ( pQ->x * pQ->z + pQ->y * pQ->w ));
	pOut->m[9] = scale->y * (2.0f * ( pQ->y * pQ->z - pQ->x * pQ->w ));
	pOut->m[10] = scale->z * (1.0f - 2.0f * ( pQ->x * pQ->x + pQ->y * pQ->y ));
	pOut->m[11] = 0.0f;

	// Fourth row
	pOut->m[12] = translation->x;
	pOut->m[13] = translation->y;
	pOut->m[14] = translation->z;
	pOut->m[15] = 1.0f;
