...
// add by zhuyi 2014-08-25
Vec3* Quaternion::QuaternionMultiplyVec3(Vec3* pOut, const Quaternion* q, const Vec3* v) 
{
	Vec3 uv, uuv, qvec;
...
Quaternion* Quaternion::QuaternionRotationBetweenVec3(Quaternion* pOut, const Vec3* vec1, const Vec3* vec2, const Vec3* fallback) 
{
	float a;
	...
	if (a < (1e-6f - 1.0f))
	{
		if (fabs(fallback->lengthSquared()) < kmEpsilon)
		{
			Quaternion::QuaternionRotationAxis(pOut, fallback, kmPI);
		}
		else 
		{
			Vec3 axis;
			...
			Vec3::cross(X, *vec1,&axis);

			//If axis is zero
			if (fabs(axis.lengthSquared()) < kmEpsilon) {
				Vec3 Y;
				Y.x = 0.0;
				Y.y = 1.0;
				Y.z = 0.0;

				Vec3::cross(Y, *vec1,&axis);
			}

			axis.normalize();
			Quaternion::QuaternionRotationAxis(pOut, &axis, kmPI);
		}
	} else {
		float s = sqrtf((1+a) * 2);
		float invs = 1 / s;
...
Quaternion* Quaternion::QuaternionRotationAxis(Quaternion* pOut,const Vec3* pV,float angle)
{
...
//add bu zhuyi 20140-08-25 ¨¦y??¦Ì?3.2¡ã?¡À?
...
void Quaternion::scale(Quaternion* pOut,const Quaternion* pIn,float scale_)
{
	pOut->x=pIn->x*scale_;
	pOut->y=pIn->y*scale_;
	pOut->z=pIn->z*scale_;
...
Quaternion* Quaternion::QuaternionSub(Quaternion* pOut, const Quaternion* pQ1, const Quaternion* pQ2)
{
	pOut->x = pQ1->x - pQ2->x;
	pOut->y = pQ1->y - pQ2->y;
	pOut->z = pQ1->z - pQ2->z;
...
Quaternion* Quaternion::QuaternionAdd(Quaternion* pOut, const Quaternion* pQ1, const Quaternion* pQ2)
{
	pOut->x = pQ1->x + pQ2->x;
	pOut->y = pQ1->y + pQ2->y;
	pOut->z = pQ1->z + pQ2->z;
...
Quaternion* Quaternion::nlerp(Quaternion* pOut, const Quaternion* q1, const Quaternion* q2, float t)
{
	float ct = dot(q1, q2);
	if (ct < 0.0f)
	{

		Quaternion::scale(pOut, q2, -1);

		QuaternionSub(pOut, pOut, q1);
		scale(pOut, pOut, t);
		QuaternionAdd(pOut, q1, pOut);
	}
	else
	{
		QuaternionSub(pOut, q2, q1);
		scale(pOut, pOut, t);
		QuaternionAdd(pOut, q1, pOut);
	}
	pOut->normalize();
