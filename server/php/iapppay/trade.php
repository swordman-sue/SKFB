...
function testOrder() {
	...
	//下单接口
	// 取传过来的参数
	$orderReq['appid'] 			= $_GET['appid'];
	$orderReq['waresid'] 		= 1;  //$_GET['waresid'];
	$orderReq['cporderid'] 		= $_GET['cporderid'];
	$orderReq['price'] 			= floatval($_GET['price']); // 因为客户端转过来的是字符串，所以需要做转换
	$orderReq['currency'] 		= $_GET['currency'];
	$orderReq['appuserid'] 		= $_GET['appuserid'];
	$orderReq['cpprivateinfo'] 	= $_GET['cpprivateinfo'];
	$orderReq['notifyurl'] 		= $_GET['notifyurl'];	
	
	/*
    $orderReq['appid'] = "$appid";
    $orderReq['waresid'] = 1;
    $orderReq['cporderid'] = "123412zxcvzasdfqwerasdghj11"; //确保该参数每次 都不一样。否则下单会出问题。
    //echo "microtime()";
...
function H5orPCpay() {
    ...
    $orderReq['url_h'] = 'http://news.163.com'; //返回商户URL地址
