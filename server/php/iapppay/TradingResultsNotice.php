...
function testResult() 
{
	...
	else
	{
		...
		else
		{
			...
			// 组成数组
			$arr = array('arr' => $arr, 'appid' => $appid, 'appuserid' => $appuserid, 'cporderid' => $cporderid, 
			'cpprivate' => $cpprivate, 'paytype' => $paytype, 'result' => $result, 'transid' => $transid, 'transtime' => $transtime, 'waresid' => $waresid, 'money' => $money);
