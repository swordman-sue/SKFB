...
inline int CHttpResponseMaker::make(const char* szContent, int nContentLen, char* szBuffer, int nBufferSize)
{
	...
	if(nContentLen > 0)
		nRealContentLen = int(nContentLen + strlen("\r\n\r\n"));

	sprintf(szBuffer, "HTTP/1.1 200 OK\r\nContent-Length: %d\r\nContent-Type: text/html;charset=utf-8\r\nConnection: Keep-Alive\r\n\r\n",
			nRealContentLen);
	int nHeadLen = (int)strlen(szBuffer);
	if(nContentLen > 0)
	{
...
inline void CHttpResponseMaker::make_string(const string& strContent, string& strResp)
{
	...
	make(strContent.c_str(), (int)strContent.size(), szBuffer, sizeof(szBuffer));
	strResp = szBuffer;
	delete []szBuffer;
}
...
inline int CHttpLengthAnaly::get_length_ex(const char* szData, int nDataLen, int& nContentPos, int& nContentLen)
{
	...
	nContentPos = 0;
	nContentLen = 0;
	int nHeadLen = int(pHeadEnd+4-szData);
	if(bPostType || bRespType)
	{
		...
		if(pContentLen)
		{
			...
			if(pContentLenEnd)
			{
				...
				//内容的相对位置
				nContentPos = int(pHeadEnd-szData + strlen("\r\n\r\n"));
			}
		}
	}
...
public:
	...
	float get_param_float(const char* szKey);
...
inline bool CHttpParamParser::parse(const char* szHttpParam, int nLen)
{
	...
	while(1)
	{
		...
		char szTmp[4096]; // buf大小需要根据实际情况做调整
		//提取key
		memset(szTmp, 0, sizeof(szTmp));
...
inline float CHttpParamParser::get_param_float(const char* szKey)
{
	return float(atof(get_param(szKey).c_str()));
}
...
inline int CHttpParser::parse(const char* szHttpReq, int nDataLen, int nExtraParamType)
{
	...
	if(m_pszActParam)
	{
		...
		{
			m_paramParser.parse(m_pszActParam, (int)strlen(m_pszActParam));
		}
	}
