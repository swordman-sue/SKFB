...
//
// Modified by Liang Guodong ( 2010/11/11 )
///////////////////////////////////////////////////////////////////////////////
#ifndef LUAPLUSCD_H
#define LUAPLUSCD_H
...
namespace LPCD
{
	...
	inline bool Check(lua_State* L, bool value)						{  return true;  }
	inline bool Check(lua_State* L, char value)						{  return true;  }
	inline bool Check(lua_State* L, unsigned char value)			{  return true;  }
	inline bool Check(lua_State* L, short value)					{  return true;  }
	inline bool Check(lua_State* L, unsigned short value)			{  return true;  }
	inline bool Check(lua_State* L, int value)						{  return true;  }
	inline bool Check(lua_State* L, unsigned int value)				{  return true;  }
	inline bool Check(lua_State* L, long value)						{  return true;  }
	inline bool Check(lua_State* L, unsigned long value)			{  return true;  }
	inline bool Check(lua_State* L, double value)					{  return true;  }
	inline bool Check(lua_State* L, float value)					{  return true;  }
	inline bool Check(lua_State* L, const char* value)				{  return true;  }
	inline bool Check(lua_State* L, const char* value, int len)		{  return true;  }
	inline bool Check(lua_State* L, const LuaNil&)					{  return true;  }
	inline bool Check(lua_State* L, lua_CFunction value)			{  return true;  }
	inline bool Check(lua_State* L, const void* value)				{  return true;  }
	inline bool Check(lua_State* L, const LuaLightUserData& value)	{  return true;  }
	inline bool Check(lua_State* L, const LuaUserData& value)		{  return true;  }
	...
	inline bool Match(TypeWrapper<LuaNil>, lua_State* L, int idx)
	...
	struct ReturnSpecialization<void>
	{
		...
		static int Call(void (*func)(P1), lua_State* L, int index)
		{
			...
			func(Get(TypeWrapper<P1>(), L, index + 0));
			return 0;
		}
...
#ifdef LUA_RESETCOROUTINE
	template <typename Func>
	class DirectCallCoFunctionDispatchHelper
	{
	private:
		static inline void ClearPendingCoroutineInfo(lua_State* L)
		{
			lua_pushnil(L);
			lua_pushnil(L);
			lua_setfield(L, LUA_REGISTRYINDEX, REGISTRY_PENDING_COROUTINE);
			lua_setfield(L, LUA_REGISTRYINDEX, REGISTRY_PENDING_COROUTINE_YIELD);
		}
	public:
		static inline int DirectCallCoFunctionDispatcher(lua_State* L)
		{
			// init pending info
			lua_pushthread(L);
			lua_setfield(L, LUA_REGISTRYINDEX, REGISTRY_PENDING_COROUTINE);
			...
			// execute
 			unsigned char* buffer = GetFirstUpValueAsUserData(L);
			int nres = Call(*(Func*)(buffer), L, 1);

			lua_getfield(L, LUA_REGISTRYINDEX, REGISTRY_PENDING_COROUTINE_YIELD);
			if (lua_toboolean(L, -1)) {
				// yield coroutine
				lua_pop(L, 1);
				ClearPendingCoroutineInfo(L);
