...
LuaStackObject LuaObject::Push(lua_State* NL) const
{
	luaplus_assert(NL);
	InternalPushTObject(NL, &m_object);
	return LuaStackObject(NL, InternalGetTop(NL));
}
...
void LuaObject::RegisterHelper(const char* funcName, NAMESPACE_LUA_PREFIX lua_CFunction function, int nupvalues,
							   const void* callee, unsigned int sizeofCallee, void* func, unsigned int sizeofFunc)
{
	luaplus_assert(L);
	lua_lock(L);
	luaC_checkGC(L);
