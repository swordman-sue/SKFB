...
/*	template <typename T>
	...
	**/
	...
	LUAPLUS_CLASS_API LuaStackObject Push(lua_State* NL) const;
	...
	template <typename Func>
	inline void RegisterCoFunction(const char* funcName, Func func, unsigned int nupvalues = 0)
	{
		RegisterHelper(funcName, LPCD::DirectCallCoFunctionDispatchHelper<Func>::DirectCallCoFunctionDispatcher, nupvalues, NULL, 0, &func, sizeof(func));
	}
#endif
