		...
		<div class="index-contentall">
			<div class="news-content base">
				<div class="product-category">
					<ul class="fr">
						<?php if(is_array($caseclassify)): $i = 0; $__LIST__ = $caseclassify;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
							<a class="<?php if($cat_id ==$vo[cat_id]){echo 'producta-active';} ?>" href="<?php echo U('Engineering/index',array('cat_id'=>$vo[cat_id]));?>"><?php echo ($vo["name"]); ?></a>
							</li><?php endforeach; endif; else: echo "" ;endif; ?>
					</ul>
				</div>

				<div class="news-detail">
				<?php if(is_array($itemcase)): $i = 0; $__LIST__ = $itemcase;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="news-detaiT">
						<h3><?php echo ($vo["title"]); ?></h3>
						<small>发布时间：<?php echo (date("Y-m-d",$vo["date"])); ?></small>
						<small>作者：<?php echo ($vo["author"]); ?></small>
					</div>
					<div><?php echo (stripslashes(htmlspecialchars_decode($vo["content"]))); ?></div><?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
			</div>
		</div>
