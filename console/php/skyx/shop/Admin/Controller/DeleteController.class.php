...
class DeleteController extends PublicController {
    /**
     * 构造函数
     * @session   判断是否有记录
     *
    ...
    public function add(){
        ...
        $config = M('php_config')->where('type=5')->order('typeid asc')->select();
        $this->assign('config',$config);
        $server_list = M('server_list')->order('server_id asc')->select();
        $this->assign('server_list',$server_list);
        if(IS_POST){
            $_POST['cmd_id'] = $_POST['cmd_id'];
            //判断是否为空
            if(empty($_POST['param1'])){
                echo "<script>alert('排行榜类型必须选一个！');history.go(-1);</script>";exit;
            }
            if(empty($_POST['param2'])){
                echo "<script>alert('服务器ID必须选一个！');history.go(-1);</script>";exit;
            }
