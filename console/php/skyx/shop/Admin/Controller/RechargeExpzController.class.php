...
class RechargeExpzController extends PublicController {
    /**
     * 构造函数
     * @session   判断是否有记录
     *
    ...
    public function index(){
        ...
        $result = M('php_config')->where('type=8')->order('typeid asc')->limit($this->Page->firstRow.','.$this->Page->listRows)->select();
    ...
    public function edit(){
        if(IS_POST){
            ...
            if($result===false){
                ...
                $this->success('编辑成功', U('Admin/'.CONTROLLER_NAME.'/index','id='.$_POST['id']));
            }
        }else{
            //查询原本数据
