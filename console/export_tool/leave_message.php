...
<br><br><center><font color=green size=3><b>留言列表</b></font></center>
<br>
	...
	<table cellspacing=0 bordercolordark=#FFFFFF width="70%" bordercolorlight=#000000 border=1 align="center" cellpadding="2">
	  ...
	  <tr>
		<td width="30%" align="right" height="19">服务器ID(默认所有服)：</td>
...
<table cellspacing=0 bordercolordark=#FFFFFF width="70%" bordercolorlight=#000000 border=1 align="center" cellpadding="2">
  <tr bgcolor="#6b8ba8" style="color:FFFFFF">
	...
	<td width="2%" align="center" valign="bottom">联系方式</td>
	<td width="4%" align="center" valign="bottom">留言日期</td>
	<td width="15%" align="center" valign="bottom">留言内容</td>
  </tr>
<?php    
	// 取挑转过来的参数
    ...
    $select_sql = 'select role_id,contact,date,message from leave_message where server_id = '.$server_id.'';
	//if($role_id)
	//{
	//	$select_sql = 'select role_id,contact,date,message from leave_message where server_id = '.$server_id.' and role_id = '.$role_id.'';
	//}
	//else
	//{
	//	$select_sql = 'select role_id,contact,date,message from leave_message where server_id = '.$server_id.' ORDER BY cur_rank limit 0,'.$rank_max;
	//}
    ...
    while($row=mysqli_fetch_array($result)) //通过循环读取数据内容
    {
		...
		$role_name = $role_info[0] ? $role_info[0] : "无名";
?>
	  <tr> 	  
